...
def center_window(self, w, h):
    ...
    #calculate x and y coordinates to place window in center of user screen
    x = int((screen_width/2) - (w/2))
    y = int((screen_height/2) - (h/2))
    centerGeo = self.master.geometry('{}x{}+{}+{}'.format(w, h, x, y))
    return centerGeo

#catch if user clicks on window closing X
def ask_quit(self):
...
def onSelect(self,event):
    ...
    with conn:
        ...
        #this returns a tuplethat we slice in to 4 parts using data[] during iteration
        for data in varBody:
...
def addToList(self):
    ...
    #normalize the data to keep uniform in the database
    var_fname = var_fname.strip #removes any blank spaces before or after entry
    var_lname = var_lname.strip 
    var_fname = var_fname.title() #ensures first letter in each word is capitalized
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #
        ...
        with conn:
            cursor = conn.cursor()
            #Checks the database  for existance of the fullname, if so the user is alerted and request is disregarded
            ...
            if chkName == 0: #if this is 0 there is no existance of the fullname and we can add new data
                ...
                self.lstList1.insert(END, var_fullname) # updates list box with new fullname
                onClear(self) #clears all the text boxes
            else:
               messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".format(var.fullname))
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        #check count to ensure this is not the last record in the database... cannot delete the last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone() [0]
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_phonebook WHERE col_full name = '{}'""".format(var_select))
                onDeleted(self) #call the function  to clear  all of the textboxes and selected index of listbox
######              onRefresh(self) #update the listbox of the changes
                conn.commit()
        else:
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and can not be daleted at this time. \n\nPlease add another record first before you can delete ({}).".format(var_select))
    conn.close()
...
def onUpdate(self):
    ...
    var_phone = self.txt_phone.get().strip() #normalixing the data to maintain database integrity
    var_email = self.txt.email.get().strip()
    if(len(var_phone) > 0) and (len(var_email) > 0): #ensure there is data present
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cur = conn.cursor()
            #count records to see if the user's changed are already in
            ...
            print(count2)
            if count == 0 or count2 == 0: #if proposed changes are not in the database, then proceed
